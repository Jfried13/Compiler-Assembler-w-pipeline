...
/*int displayScreen(CPU_p cpu, int mem) {
  printf("\n\n\n");
	printf("\t\tWelcome to the LC-3 Simulator Simulator With Pipeline\n\n");
	...
	printf("\n");
	printf("  Select: 1)Load, 3)Step, 5)Display Mem, 6)Run, 8)Exit\n");
	return 0;
}*/
...
int checkForCollision(Register headOfPrefetch, int headPos, Register collisionCheck, int collisionPos) {
	...
	if (Rd == Rs1 || (Rs2 >= 0 && Rd == Rs2)) {  //Checks if Rd is Rs1 or that Rd is Rs2 as long as Rs2 is not a sext.
		nopCount = 4 - (collisionPos - headPos) + 1;
	}
				...
				if (cpu->buffers[3].Opcode == LDI || cpu->buffers[3].Opcode == STI) {
					...
					switch (cpu->buffers[3].Opcode) {
						...
						case LEA:
							...
							break;
						case TRAP:
							value = trap(cpu, cpu->buffers[3].B);
							cpu->r[7] = cpu->buffers[3].PC;
							//start NOP stall
							if (value == 1) {
								//displayScreen(cpu, 0);
								//printf("end of file\n");
...
int main(int argc, char* argv[]){
	...
	memShift = 0;
	//FILE *inputFile;
	//inputFile = fopen("getc.hex", "r");
	char *temp;
	int dontDeleteThisGarbage;
	...
	//controller(cpu, 1);
	//displayScreen(cpu, 0, 0, 0, 0, 0, temp);
